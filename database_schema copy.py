...
class AlertDatabase:
    ...
    def create_alert(self, email: str, symbol: str, threshold_percent: float) -> Dict:
        ...
        with sqlite3.connect(self.db_file) as conn:
            ...
            print(f"🎯 目標価格: ${base_price * (1 + threshold_percent/100):,.6f}")
...
def main():
    """データベース初期化とテスト"""
    print("🗄️ CryptoAlert Database 初期化（Binance API連携版）")
    print("=" * 60)
    ...
    try:
        ...
        for alert in active_alerts:
            ...
            print(f"  • {alert['symbol']}: {alert['threshold_percent']:+.2f}% 目標")
            print(f"    現在: ${current_price:,.6f} ({change:+.2f}%) - {alert['email']}")
        
        print("\n🔍 アラート条件チェックテスト...")
        for alert in active_alerts[:2]:  # 最初の2つだけテスト
            result = db.check_alert_condition(alert)
            if result:
                status = "🚨 発火!" if result['triggered'] else "⏳ 待機中"
                print(f"  • {result['symbol']}: {result['price_change']:+.2f}% {status}")
